...
func (siw *ServerInterfaceWrapper) GetSession(c *fiber.Ctx) error {
	...
	if value, found := headers[http.CanonicalHeaderKey("traceparent")]; found {
		var Traceparent Traceparent

		err = runtime.BindStyledParameterWithLocation("simple", false, "traceparent", runtime.ParamLocationHeader, value, &Traceparent)
		if err != nil {
			return fiber.NewError(fiber.StatusBadRequest, fmt.Errorf("Invalid format for parameter traceparent: %w", err).Error())
		}
	...
	if value, found := headers[http.CanonicalHeaderKey("tracestate")]; found {
		var Tracestate Tracestate

		err = runtime.BindStyledParameterWithLocation("simple", false, "tracestate", runtime.ParamLocationHeader, value, &Tracestate)
		if err != nil {
			return fiber.NewError(fiber.StatusBadRequest, fmt.Errorf("Invalid format for parameter tracestate: %w", err).Error())
		}
